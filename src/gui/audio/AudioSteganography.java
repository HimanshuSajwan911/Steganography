...
import static steganography.core.util.Files.setAudioFileExtension;

/**
 * @author Himanshu Sajwan.
 */

public class AudioSteganography extends gui.stegano.Steganography{
    ...
    protected int encode(){
        ...
        steganography.AudioSteganography steg = new steganography.AudioSteganography();
    ...
    protected int decode(){
        ...
        steganography.AudioSteganography audio_steganography = new steganography.AudioSteganography();
       
        if(OFFSET != -1){
            audio_steganography.setOffset(OFFSET);
        }
        
        if(BUFFER_CAPACITY != -1){
            audio_steganography.setBufferCapacity(BUFFER_CAPACITY);
        }
        
        try {
            audio_steganography.decode(COVER_FILE.getPath(), DESTINATION_FILE.getPath(), KEY);
