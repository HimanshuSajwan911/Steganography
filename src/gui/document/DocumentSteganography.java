...
import static steganography.core.util.Files.setDocumentFileExtension;

/**
 * @author Himanshu Sajwan.
 */

public class DocumentSteganography extends gui.stegano.Steganography{
    ...
    protected void openCoverFile() {
        ...
        chooser.setDialogTitle("Open Document File.");
        chooser.setAcceptAllFileFilterUsed(false);
        setDocumentFileExtension(chooser);
    ...
    protected int encode(){
        ...
        steganography.DocumentSteganography document_steganography = new steganography.DocumentSteganography();
        
        if(OFFSET != -1){
            document_steganography.setOffset(OFFSET);
        }
        
        if(BUFFER_CAPACITY != -1){
            document_steganography.setBufferCapacity(BUFFER_CAPACITY);
        }
        
        try {
            document_steganography.encode(COVER_FILE.getPath(), DATA_FILE.getPath(), DESTINATION_FILE.getPath(), KEY);
    ...
    protected int decode(){
        ...
        try {
            document_steganography.decode(COVER_FILE.getPath(), DESTINATION_FILE.getPath(), KEY);
